...
- **순서 결정(2026-07-24)**: `T-10`(동시성 테스트) → `T-11`(실 서버) → `T-12`(CI/CD) 뒤로 미룸. 이유: 사용자가 CRUD는
  이미 많이 해봐서 이 프로젝트에서 반복할 필요가 없고, CI/CD가 진짜 미경험 영역이라 학습 목적상 그쪽을 먼저 파는 게
  맞다고 판단. CI/CD 파이프라인은 보통 "빌드→배포"만 하고 앱 내부 CRUD 로직엔 거의 영향받지 않아서, 순서를
  뒤집어도 나중에 파이프라인을 다시 고칠 리스크는 작음
- **최종 순서 재확정(2026-07-26)**: `T-11` 완료 직후 `T-12`(CI/CD) → `T-10`/`T-04` 테스트 마무리 → `T-08`(이 항목)
  순으로 확정. `T-10`(특히 낙관적 락 동시성 테스트)이 이 프로젝트 핵심 학습 목표라, CRUD가 얹는 새 복잡도(FK 없는
...
- **착수 시작 (2026-07-28)**: GitHub Actions로 CD(배포 자동화)만 우선 구성 — `.github/workflows/deploy.yml`(build-and-push job: 이미지 빌드 후 GHCR push / deploy job: SSH로 EC2 접속해 `pull`+`up -d`), `docker-compose.prod.yml`(override로 `app` 서비스가 GHCR 이미지를 쓰게 함). 이미지 빌드는 서버가 아니라 GitHub Actions 러너에서 하고 GHCR(public)에 올린 뒤 서버는 pull만 하는 방식으로 결정 — 프리티어(`t2.micro`, RAM 1GB) 서버가 직접 빌드하면 부하가 커서. Jenkins는 사용자가 별도로 배워보고 싶다고 해서 이번 스코프에서 제외(나중에 별도 학습). nginx blue/green 무중단 배포는 이번 스코프에서 제외하고 다음 단계로 미룸
- **CI(자동 테스트)는 의도적으로 미포함 (2026-07-28)**: 지금 만든 파이프라인은 빌드+배포까지만 자동화된 CD고, 진짜 CI(자동 테스트)는 아직 없음. `T-10`/`T-04`의 JUnit 테스트가 충분히 채워지면, `deploy.yml`에 `build-and-push`보다 먼저 도는 `test` job(`./gradlew test`)을 추가해서 테스트 실패 시 배포 자체를 막도록 CI 환경을 다시 구성하기로 함
