...
### T-08. 관리자 CRUD — 극장/영화/스케줄 등록, 좌석 배치 커스터마이징
...
- **현재**: 테스트 코드 0개. 지금까지 전부 curl/브라우저 수동 검증만 함
- **목표**: 지금까지 구현된 로직(특히 동시성 제어)을 자동화된 테스트로 관리
- **계획된 4단계** (한 번에 다 안 하고 순서대로, 매 단계 확인받으며 진행):
  1. JUnit 기초 문법 — `@Test`, assertion, `./gradlew test` 실행법
  2. 순수 로직 테스트 — `SeatService`의 상태 검증(예외 던지는 경로) 위주
  3. Mockito로 의존성 모킹 — `SeatMapper`를 가짜로 대체해서 `holdPessimistic`/`holdOptimistic` 성공/충돌 경로 테스트
  4. 동시성 통합 테스트 — 진짜 DB 위에서 여러 스레드가 동시에 같은 좌석을 잡을 때 대기형 vs 충돌감지형 락이 실제로 어떻게 다른지 검증 (이 프로젝트 핵심 학습 대상)
